...
S4Vectors::setValidity2("DiffEmbFit", function(obj){
  ...
  if(! is.null(ambient_coordsystem) && ncol(ambient_coordsystem) != n_ambient) msg <- c(msg, "`ncol(ambient_coordsystem)` does not match `n_ambient`")
  ...
  if(! is.null(linear_coefficients) && nrow(linear_coefficients) != n_ambient) msg <- c(msg, "`nrow(linear_coefficients)` does not match `n_ambient`")
  ...
  if(! is.null(diffemb_basepoint) && nrow(diffemb_basepoint) != n_ambient) msg <- c(msg, "`nrow(diffemb_basepoint)` does not match `n_ambient`")
  ...
  if(! is.null(diffemb_coefficients) && dim(diffemb_coefficients)[1] != n_ambient) msg <- c(msg, "`dim(diffemb_coefficients)[1]` does not match `n_ambient`")
