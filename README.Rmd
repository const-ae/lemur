...
The `test_de` function takes a `lemur_fit_obj` and returns a matrix for all genes and cells containing the difference between two conditions specified in the `contrast`. Note that `lemur` implements the notation for contrasts. Instead of providing a contrast vector or providing the design matrix column names, you provide for each *condition* the expected levels and `lemur` automatically calculates the contrast vector.
```{r lemur_test_de}
