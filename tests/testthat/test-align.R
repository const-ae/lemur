...
test_that("check that harmony alignment works as expected", {
  # The maximum diversity clustering step itself (init_max_diversity_clustering /
  # run_max_diversity_clustering) is validated against real harmony 1.2.4 output
  # in test-max_diversity_clustering.R. This test just checks the functional
  # behavior of align_harmony(): that points from different conditions end up
  # close together in the aligned embedding.
  set.seed(1)
  n_genes <- 10
  n_emb <- 2
  n_points <- n_emb + 1
